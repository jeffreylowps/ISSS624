...
```{r}
pacman::p_load(rgdal, spdep, tidyverse, tmap, sf, ggpubr, cluster, factoextra, NbClust, heatmaply, corrplot, psych, ClustGeo)
```
...
In this section, we will gain hands-on experience on using functions provided by ClustGeo package to perform non-spatially constrained hierarchical cluster analysis.

### Ward-like Hierarchical Clustering: ClustGeo

ClustGeo package provides function like hclustgeo() to perform a typical Ward-like hierarchical clustering just like hclust().
...
Note that the dissimlarity must be an object of class dist, i.e. an object obtained with the fuction dist().

### Mapping the clusters formed

Similarly, we can plot the clusters on a catergorical area shaded map by using the steps in we learned in 5.7.12 Mapping the clusters formed.
...
Next, the choicealpha() will be used to determine a suitable value for the mixing parameter alpha as shown in code chunk below.
...
Next, cutree() is used to determine the cluster object.
